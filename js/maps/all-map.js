...
function getLocation(ffsXmlLoc, cpdsXmlLoc, isAddable, isGetNames){	
	if(fileExists(ffsXmlLoc)){
		ffsXml = loadXml(ffsXmlLoc);
		if(isGetNames == true){
			getCPDS(ffsXml);
		}
	}else{
		alert("FFS XML File Not Found!");
	}
	
	if(fileExists(cpdsXmlLoc)){
		cpdsXml = loadXml(cpdsXmlLoc);
		if(isGetNames == true){
			getCPDS(cpdsXml);
		}
	}else{
		alert("CPDS XML File Not Found!");
	}
...
function createMarkers(ffsXml, cpdsXml) {
	var cpds = cpdsXml.getElementsByTagName('station');
	...
	var ffs = ffsXml.getElementsByTagName('fireworksFact');
	...
	for (var i = 0; i < ffs.length; i++) {
		var latlng = new google.maps.LatLng(
			ffs[i].getElementsByTagName('lat')[0].childNodes[0].nodeValue,
			ffs[i].getElementsByTagName('long')[0].childNodes[0].nodeValue);
